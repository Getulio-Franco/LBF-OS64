...
void process_spawn_pending() {
    ...
    pid_do_pai = pai_solicitante_pid; // Captura o PID de quem pediu
    ...
    // O disco lê o ELF de forma segura com interrupções ativas
    uint64_t pid_filho = create_elf_process(caminho_local);

    // 🔥 GESTÃO DE FOCO UNIFICADA E AUTOMÁTICA:
    if (pid_filho != (uint64_t)-1) {
        ...
        if (proc_filho) {
            ...
            // Se o pai era o processo em foreground (ex: o Terminal), 
            // passamos o controle de entrada diretamente para o filho!
            if (proc_pai && proc_pai->is_foreground == 1) {
                proc_filho->is_foreground = 1;
                proc_pai->is_foreground = 0;
                foreground_process = proc_filho;
                
                // Limpa o buffer de teclado para o novo app iniciar limpo
                while(keyboard_pop_char() != 0); 
            }
            // Se o pai NÃO era o foreground (ex: o Gerenciador de Tarefas disparando o app),
            // o processo nasce em background e o foco continua com quem já estava.
        }
    }
}
