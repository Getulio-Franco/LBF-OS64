...
uint64_t syscall_handler(uint64_t syscall_num, uint64_t arg1, uint64_t arg2, 
    ...
    switch (syscall_num) {
        ...
        case SYS_CLEAR:         
            terminal_clear(); 
            refresh_screen = 1; // Garante que a GUI de fundo volte após o clear
            return 0;
        ...
        case SYS_MKDIR:
            return (arg1) ? (uint64_t)fat32_create_directory((const char*)arg1) : (uint64_t)-1;
...
case SYS_EXEC: { // ID 19 - Unificado e Assíncrono via task_d
    ...
    if (!tem_pedido_exec) {
        ...
        int i = 0;
        // Copia de forma segura caractere por caractere para o buffer global
        while (src[i] != '\0' && i < 127) {
        ...
        // Salva o processo atual que chamou a syscall como PAI para a task_d gerenciar o foco depois
        ...
        __asm__ volatile("sti");
        return 0; // Pedido aceito com sucesso! A task_d vai processar em background.
    }
    __asm__ volatile("sti");
    return (uint64_t)-2; // Sistema ocupado (fila de spawn cheia)
}
...
//=================
        ...
        case SYS_TESTINIT: { // Substitua pelo número/macro correto da sua syscall
