...
void draw_bitmap(int x, int y, int w, int h, uint32_t* data) {
    ...
    for (int j = 0; j < h; j++) {
        // Verifica se a linha j está dentro da tela
        if ((y + j) < 0 || (y + j) >= (int)screen_height) continue;

        for (int i = 0; i < w; i++) {
            // Verifica se a coluna i está dentro da tela
            if ((x + i) < 0 || (x + i) >= (int)screen_width) continue;

            uint32_t color = data[j * w + i];
            // Transparência Magenta
            if (color != 0xFF00FF) {
                uint32_t* pixel = (uint32_t*)ram_buffer + (y + j) * screen_width + (x + i);
                *pixel = color;
            }
        }
    }
