...
void ehci_detectar_dispositivos(void) {
    ...
    for (uint8_t i = 0; i < n_ports; i++) {
        ...
        if (port_status & EHCI_PORT_CONNECTED) {
            ...
            // Verifica se o reset deu certo (retorna 1)
            if (ehci_reset_porta(i)) {
                
                // ++ LINHAS IMPLANTADAS AQUI ++
                ehci_wait_ms(100); // Dá um tempo vital para o hardware real estabilizar
                ehci_ler_identificacao(); // Inicia o envio de pacotes SETUP e pega os dados!
