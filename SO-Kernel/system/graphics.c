...
// CORREÇÃO 3: Posicionamento de pixels matemático exato
void sys_draw_put_pixel(int x, int y, uint32_t color) {
    if (!ram_buffer) return;
    if (x < 0 || x >= screen_w || y < 0 || y >= screen_h) return;
    
    // Calcula o offset exato em bytes
...
// CORREÇÃO 1: Inicialização lendo o Pitch e BPP reais
void graphics_init(void) {
    ...
    // A MÁGICA ESTÁ AQUI: Pegar os dados reais da VESA
    bpp_bytes = info.bpp / 8;
    if (bpp_bytes == 0) bpp_bytes = 4; // Fallback de segurança
    
    screen_pitch = info.pitch;
    if (screen_pitch == 0) screen_pitch = screen_w * bpp_bytes; // Fallback
    
    size_t buffer_size = (size_t)screen_pitch * screen_h;
    ram_buffer = (uint8_t*)malloc(buffer_size);
    
    if (!ram_buffer) sys_exit();
...
// CORREÇÃO 2: Limpar tela usando o Pitch correto linha por linha
void graphics_clear(uint32_t color) {
    if (!ram_buffer) return;
    
    // Usar 'total_pixels' ignora o padding. O certo é pular o pitch a cada linha Y.
    for (int y = 0; y < screen_h; y++) {
        ...
        } else if (bpp_bytes == 3) { // Suporte a 24bpp (QEMU costuma fazer isso)
            for (int x = 0; x < screen_w; x++) {
                int offset = x * 3;
