...
void kernel_main() {
    ...
    // ========================================================================
    // CORREÇÃO PONTO A PONTO: SATA E SISTEMA DE ARQUIVOS NO MOMENTO DE REPOUSO
    // ========================================================================
    if (ahci_hal_inicializar()) {
        //  fat32_init();
        // Substitua o antigo fat32_init() por:
        storage_init();
        storage_register_device(0, "SATA HDD", 200000, 512, (storage_read_func_t)ahci_hal_ler, NULL);
        fat32_mount(0); // Monta o SATA imediatamente no boot!
    } else {
        terminal_print("Kernel: Abortando montagem do FS devido a falha no barramento SATA.\n");
    }
