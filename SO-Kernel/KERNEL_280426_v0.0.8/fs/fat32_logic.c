...
#include "drivers/hw/ahci_hal.h"  // Transição do barramento legado para o HAL SATA
...
// Se você tiver uma função abstrata de escrita implementada, declare-a aqui (Ex: storage_write_sectors)

/* Protótipo da função de escrita declarada no stub/hal */
...
uint32_t fat32_get_next_cluster(uint32_t current_cluster) {
    ...
    uint32_t fat_sector = fat_start_sector + (current_cluster / 128);
    uint32_t fat_index = current_cluster % 128;

    // ATUALIZADO: barramento dinâmico via storage_read_sectors
    if (storage_read_sectors(fat32_current_dev_id, fat_sector, 1, sector_buffer) == 0) 
        return FAT32_BAD_CLUSTER;

    uint32_t next_c = ((uint32_t*)sector_buffer)[fat_index] & 0x0FFFFFFF;
    if (next_c >= 0x0FFFFFF8) return FAT32_EOF;
    return next_c;
}

/**
 * @brief Sincroniza a escrita em ambas as cópias da FAT (Primária e Backup)
 */
void fat32_set_cluster_entry(uint32_t cluster, uint32_t value) {
    ...
    // Nota: Deixamos a leitura atualizada para evitar quebras se você chamar a função.
    if (storage_read_sectors(fat32_current_dev_id, fat_sector, 1, sector_buffer) == 1) {
        ...
        // Quando você construir a função storage_write_sectors, aplique-a aqui para salvar as alterações.
    }
}
