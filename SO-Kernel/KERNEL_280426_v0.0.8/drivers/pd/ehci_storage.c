...
int ehci_storage_init(uint8_t addr, uint32_t* out_max_lba, uint32_t* out_block_size) {
    ...
    // 1. OBRIGATÓRIO: Avisar ao pendrive para ligar os Endpoints Bulk (Configuração 1)
    usb_setup_packet_t setup;
    setup.bmRequestType = 0x00; // Host to Device, Standard, Device
    setup.bRequest = 0x09;      // SET_CONFIGURATION
    setup.wValue = 0x0001;      // Valor da Configuração = 1 (Padrão para 99% dos pendrives)
    setup.wIndex = 0x0000;
    setup.wLength = 0x0000;

    // Usamos a função do Core para mandar o comando no Endpoint 0
    if (!ehci_enviar_controle(addr, &setup, NULL, 0)) {
        terminal_print("[USB STORAGE] FALHA: Nao foi possivel definir a configuracao.\n");
        return 0;
    }

    ehci_wait_ms(50); // Dá um tempo para o pendrive energizar as portas Bulk internamente
    ...
    // Matriz expandida: Adicionamos o {0x01, 0x81} que é o mais comum no mundo real e QEMU/VBox
    ...
    for (int p = 0; p < 3; p++) {
        ...
        // Sempre que trocamos de Endpoints, resetamos o sincronismo
        toggle_out = 0;
        toggle_in = 0;
...
int ehci_storage_read_sectors(uint8_t addr, uint32_t lba, uint32_t count, void* buffer) {
    ehci_cbw_t cbw;
    memset(&cbw, 0, sizeof(ehci_cbw_t));
    ...
    return ehci_bot_transfer(addr, &cbw, buffer, total_bytes);
}

int ehci_storage_write_sectors(uint8_t addr, uint32_t lba, uint32_t count, const void* buffer) {
    ehci_cbw_t cbw;
    memset(&cbw, 0, sizeof(ehci_cbw_t));
    ...
    return ehci_bot_transfer(addr, &cbw, (void*)buffer, total_bytes);
}
