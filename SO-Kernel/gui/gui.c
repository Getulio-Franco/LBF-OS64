...
void gui_set_prop(void* control_ptr, TGUIProperty prop, uint64_t value) {
    ...
    switch (prop) {
        // =====================================================================
        // ADICIONADO: CONTROLE DE ESTADO GRÁFICO (UP/DOWN/HOVER) - CORRIGIDO
        // =====================================================================
        case PROP_STATE: {
            // Descomentamos a linha para salvar o estado real no componente!
            // Se o seu botão usa a estrutura TControl básica:
            ctrl->State = (int)value; 
            
            // Se o campo se chamar 'State' dentro de TWinControl, use a de baixo:
            // TWinControl* win_ctrl = (TWinControl*)control_ptr;
            // win_ctrl->State = (int)value; 
        ...
        case PROP_SET_FOCUS: {
            // 1. Remove o foco do controle antigo se ele existir
            if (g_focused_control) {
                TWinControl* old_win = (TWinControl*)g_focused_control;
                old_win->Focused = false;
            }
            
            // 2. Aponta a global do Kernel para o novo controle
            g_focused_control = ctrl;
            
            // 3. Marca o novo controle como focado internamente
        ...
        case PROP_CAPTION: {
            ...
            if (src) {
                ...
                // Proteção do Memo: Trabalha SÓ com o endereço de memória, não copia texto!
                else if (ctrl->Type == TYPE_MEMO) {
                    TMemo* memo = (TMemo*)control_ptr;
                    memo->TextPointer = src; 
                } 
                else {
                    // CORREÇÃO: Voltamos ao strcpy clássico do seu Kernel.
                    // Isso não vai zerar e corromper o Width/Height e o OnPaint!
        ...
        // =====================================================================
        // ADICIONADO: SINCRONIZAÇÃO DO SCROLL VERTICAL DO TMEMO (ID 65)
        // =====================================================================
        case 65: { // Correspondente ao PROP_SCROLL_Y definido no os_memo.h
            if (ctrl->Type == TYPE_MEMO) {
                TMemo* memo = (TMemo*)control_ptr;
                memo->ScrollY = (int)value; // Atualiza a linha base de renderização no Kernel
            }
            break;
        }
    ...
    // REMOVIDO DAQUI o bloco "if (ctrl->OnPaint)": 
    // Deixamos o Kernel decidir o momento certo de pintar no ciclo global da GUI!
}
