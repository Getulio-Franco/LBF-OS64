...
void wm_handle_mouse_event(int x, int y, int event) {
    ...
    switch (event) {
        case EVENT_MOUSE_DOWN: { // Corrigido: Nome real do evento mapeado
            if (!target) return;
            ...
            if (target->BorderStyle == bsNone) {
                ...
                if (y >= target->Win.Control.Top && y <= target->Win.Control.Top + title_height) {
                    if (target->Win.Draggable) { 
                        dragging = 1;
                        target->IsDragging = 1;
                        target->DragOffsetX = x - target->Win.Control.Left;
                        target->DragOffsetY = y - target->Win.Control.Top;
